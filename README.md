...
- **legislation.gov.uk**: Downloads legal documents from the 2025 Atom data feed. The pipeline follows `rel="next"` links to crawl all entries.
...
- **Enrichment**: Adds metadata such as [EU Data Theme](https://op.europa.eu/en/web/eu-vocabularies/concept-scheme/-/resource?uri=http://publications.europa.eu/resource/authority/data-theme) tags, language detection, word and token counts.
...
Whole datasets and their embeddings are published to Hugging Face for access by downstream applications. The published data and their details can be found on the [National Data Library Hugging Face collection page](https://huggingface.co/collections/hkir-dev/ndl-core-collection).
